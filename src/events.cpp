...
glm::vec2 EventHandler::GetMousePos() {
  return glm::vec2(glm::radians((float)mouseX), glm::radians((float)mouseY));
}
...
void EventHandler::processEvents() {
  ...
  while (SDL_PollEvent(&event)) {
    ...
    case SDL_MOUSEWHEEL: {
      ...
      // m->y no longer reliable (often y is 0 when mouse wheel is spun up or
      // down), use m->preciseY instead
      SDL_MouseWheelEvent *m = (SDL_MouseWheelEvent *)&event;
#ifdef EVENTS_DEBUG
      printf("SDL_MOUSEWHEEL= x,y=%d,%d preciseX,preciseY=%f,%f\n", m->x, m->y,
             m->preciseX, m->preciseY);
#endif
